...
#To do: line#74: cannot concatenate 'str' and 'int'
...
class Gene:
	...
	def __init__(self, sample_ID, gene_name, Output_geno, amplicon_name, Active_score, Drug_action, Low_coverage, Range, Gene_KB):
		...
		self.phenotype = self.get_pheno(Active_score)
	...
	def get_pheno(self, Active_score):
		...
		phenotype = self.parse_pheno(alleles, score_allele, score_string, score_threshold)
		return phenotype

	def parse_pheno(self, alleles, score_allele, score_string, score_threshold):

		#score_allele = {item.split(': ')[0] : item.split(': ')[1] for item in score_string.split(';')} ## extract socre for each allele
		#print [score_allele[allele.replace('xN', '')]*2 if 'xN' in allele else score_allele[allele] for allele in alleles] ## score of input genotype
		#print sum([score_allele[allele.replace('xN', '')]*2 if 'xN' in allele else score_allele[allele] for allele in alleles]) ## score of input genotype
