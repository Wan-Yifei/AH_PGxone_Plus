...
def Control_check():
    ...
    for control in control_genotype.keys():
        if 'NA17244' in control:
            if control_genotype[control][16] != '*2xN/*4':
...
## 2.8 Check the homo/heter of CYP2D6
def CYP_check():
    CYP_genotypes = {line.strip().split('\t')[0]:line.strip().split('\t')[1] for line in CYP_checklist if 'sample_ID' not in line}
    print('====================================================================')
...
#     Drug_info = 'T:/PGxOne_V3/Production/BI_Data_Analysis/%s/PGxOneV3_drug_action.txt'%folder ## gene and corresponding ICD codes
    ...
    os.system('chmod 777 %s_QC.txt'%Run)
