...
def ParseArg():
    ...
    subparsers = p.add_subparsers(help="All available sub commands")
...
def SendPDF_PWN(PDF_path, PWN_req, abnormal=True):
    ...
    response = L.CreateResults(PWN_req, data, PDF_path)
    print response
...
def FindActionFile(reqID):
    command = 'find /data/CLIA-Data/PGxOne_V3/Production/BI_Data_Analysis/18*/Plus -mtime 0 | grep "LIS.txt" | grep "%s"'%(reqID)
    run = os.popen(command)
    ...
    if actionFile.strip()=="":
        ...
        return None
    command = 'sed "1 i\##RequisitionNumber: %s" %s> /tmp/%s'%(reqID, actionFile, actionFile.split("/")[-1])
    os.system(command)
...
def run(args):
    ...
    for report in daily_reports:
        ...
        SentPWN = SendPDF_PWN(args.reportFolder+"/"+report, PWN_req)
        if SentPWN:
        ...
        ActionFile = FindActionFile("A-"+PWN_req)
        if ActionFile != None:
            response = SendFlatFile(ActionFile)
            if not response.ok:
        ...
        if PatientInfoFile != None:
            response = SendFlatFile(PatientInfoFile)
            if not response.ok:
